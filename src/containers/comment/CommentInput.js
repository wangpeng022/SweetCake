...
export default class CommentInput extends React.Component{
    ...
    handleSubmit=(event)=>{
        ...
        if(user){
            let username= user.id;//获取用户输入的用户名
    ...
    render(){
        return(
            <div>
                <form className="form-horizontal" onSubmit={this.handleSubmit}>{/*form-horizontal文本与输入框在一行上  ,这里用onSubmit而不是用onclick绑定,是为了检测表单的输入值的合法性,只有校验通过了才会提交*/}
